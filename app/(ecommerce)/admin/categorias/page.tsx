...
export default function AdminCategoriasPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.error || error.message || 'Error al guardar categoría'
  ...
  const handleDelete = async (categoria: Categoria) => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.error || error.message || 'Error al eliminar categoría'

      // Si hay productos asociados, mostrar mensaje específico
      if (error.response?.data?.productosAsociados !== undefined) {
        toast.error(
          `No se puede eliminar. Hay ${error.response.data.productosAsociados} producto(s) usando esta categoría. Re-asigná los productos primero.`
        )
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center justify-end gap-4 border-t border-gray-200 pt-4">
                <button
                  ...
                  className="rounded-lg border border-gray-300 px-6 py-2 font-semibold text-gray-700 transition-all hover:bg-gray-50"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  className="rounded-lg bg-black px-6 py-2 font-semibold text-white transition-all hover:bg-gray-800"
                >
