...
export default function ProductoClient() {
  ...
  const images = [
    ...
    .filter(Boolean)
    .map((img: string) => img || '/images/default-product.svg')
    ...
    <main className="min-h-screen bg-white">
      <div className="container mx-auto px-4 py-8 md:py-12">
        ...
        <div className="grid gap-8 md:grid-cols-2 md:gap-12">
          ...
          >
            <div>
              <h1 className="mb-2 text-3xl font-bold text-black md:text-4xl">{product.nombre}</h1>
            ...
            <div className="flex items-center gap-4">
              {product.descuento ? (
                ...
                <span className="text-3xl font-bold text-black">{formatPrice(product.precio)}</span>
              )}
            </div>

            {product.descripcion && (
              <div>
                <h3 className="mb-2 font-semibold text-black">Descripción</h3>
                <p className="leading-relaxed text-gray-600">{product.descripcion}</p>
              </div>
            )}

            <div>
              <h3 className="mb-2 font-semibold text-black">Color</h3>
              <p className="capitalize text-gray-600">{product.color || 'N/A'}</p>
            </div>
            ...
            <div className="space-y-3 pt-4">
              <button
                ...
                className="flex w-full transform items-center justify-center gap-2 rounded-lg bg-black py-4 font-semibold text-white transition-all hover:scale-[1.02] hover:bg-gray-800 disabled:cursor-not-allowed disabled:bg-gray-400"
              >
              ...
              {product.idMercadoPago ? (
                <button
                  ...
                  disabled={getStockStatus(product, selectedTalle) === 'agotado'}
                  className="flex w-full transform items-center justify-center gap-2 rounded-lg bg-blue-600 py-4 font-semibold text-white transition-all hover:scale-[1.02] hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-gray-400"
                >
                  <CreditCard size={20} />
                ...
                <div className="group relative">
                  <button
                    disabled
                    className="flex w-full cursor-not-allowed items-center justify-center gap-2 rounded-lg bg-gray-300 py-4 font-semibold text-gray-500"
                  >
                    <CreditCard size={20} />
                    Comprar con Mercado Pago
                  </button>
                  <div className="pointer-events-none absolute bottom-full left-1/2 mb-2 -translate-x-1/2 whitespace-nowrap rounded-lg bg-black px-3 py-2 text-sm text-white opacity-0 transition-opacity group-hover:opacity-100">
